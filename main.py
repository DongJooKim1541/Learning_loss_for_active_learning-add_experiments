...
def train(models, criterion, optimizers, schedulers, dataloaders, num_epochs, epoch_loss, plot_data):
    ...
    print('>> Finished.')
    print('\n Training time: %.2f min' % ((time.time() - start_time)/60))
...
if __name__ == '__main__':
    ...
    for trial in range(TRIALS):
        ...
        for cycle in range(CYCLES):
            ...
            # Training and test
            train_time=train(models, criterion, optimizers, schedulers, dataloaders, EPOCH, EPOCHL, plot_data)
            acc = test(models, dataloaders, mode='test')
            print('Trial {}/{} || Cycle {}/{} || Label set size {}: Test acc {}'.format(trial+1, TRIALS, cycle+1, CYCLES, len(labeled_set), acc))
            if trial==1:
                trial1_acc.append(acc)
                trial1_time.append(train_time)
            elif trial==2:
                trial2_acc.append(acc)
                trial2_time.append(train_time)
            else:
                trial3_acc.append(acc)
                trial3_time.append(train_time)
        ...
        #trial_mean_acc=[(trial1_acc[i]+trial2_acc[i]+trial3_acc[i]) / 3 for i in range(0)]
        #trial_mean_time = [(trial1_time[i] + trial2_time[i] + trial3_time[i]) / 3 for i in range(0, 10)]

    f = open('./AccLoss.txt', 'a')
    f.write("Trial 1: ")
    f.write(str(trial1_acc) + "\n")
    f.write(str(trial1_time) + "\n")
    f.write("Trial 2: ")
    f.write(str(trial2_acc) + "\n")
    f.write(str(trial2_time) + "\n")
    f.write("Trial 3: ")
    f.write(str(trial3_acc) + "\n")
    f.write(str(trial3_time) + "\n")
    f.write("\n")
    #f.write("Trial mean: ")
    #f.write(str(trial_mean_acc) + "\n")
    #f.write(str(trial_mean_time) + "\n")